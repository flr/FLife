...
```{r, echo=TRUE, fig.cap="Age-vectors of growthm natural mortality, maturity and selection pattern",eval=FALSE}
library(FLRP)
eql=lhEql(par)
...
```{r, fig.cap="Equilibrium curves and reference points.",eval=FALSE}
plot(eql)
```
...
```{r fig3,fig.cap="Stock recruitment relationships for a steepness of 0.75 and vigin biomass of 1000",eval=FALSE}
library(reshape)
...
```{r fig4,fig.cap="Production curves, Yield v SSB, for a steepness of 0.75 and vigin biomass of 1000.",eval=FALSE}
...
```{r fig5,eval=!TRUE,fig.height=3,fig.width=6,eval=FALSE}
...
```{r, echo=TRUE, m-density-dependence,fig.cap="Density Dependence in M",eval=FALSE}
library(FLRP)
library(FLife)
...
```{r, echo=TRUE, Maturity-density-dependence,fig.cap="Density Dependence in M",eval=FALSE}
scale=stock.n(hutchen)[,25]%*%stock.wt(hutchen)
scale=(stock.n(hutchen)%*%stock.wt(hutchen)%-%scale)%/%scale
...
```{r fig12,fig.width=6,fig.height=6,eval=FALSE,fig.cap="Year effects",eval=FALSE}
dat=rbind(cbind("Selection"="Juvenile",fishJuv),
          cbind("Selection"="Mature",  fishMat))
dat=transform(dat,F=factor(paste("F times",c(0,1,3))[iter]))
...
```{r fig13, fig.width=10,fig.height=4,eval=FALSE,fig.cap="Cohort Effects",eval=FALSE}
library(FLAssess)

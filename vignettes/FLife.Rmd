...
eq      =lhEql(alb,m=lorenzen,sr="cushing")
...
```{r}
stkr=fwd( stkr,fbar=f,sr=eq)

ref =stock.n(eq)[,1]
```

```{r fig11b, fig.height=6,fig.width=10}
scale=rnoise(1,iter(stock.wt(stkr),1),sd=0.3,b=0.9,what="cohort")
stkm=stkr
```

```{r}
m(stkm)=mdd(stock.wt(stkm),alb,scale,k) 

stkm=fwd( stkm,fbar=f,sr=eq)
```

```{r}
stkf=stkr

mat(stkf)=matdd(ages(iter(stock.wt(stkf),1)),alb,scale,k,TRUE) 
```

```{r}

stkf=fwd( stkf,fbar=f,sr=eq)
```

```{r}
stkr=fwd( stkr,fbar=f,sr=eq,
         residuals=rlnorm(3,iter(f,1)*0,.3))
...
```{r fig11c, fig.height=6,fig.width=10}
k   =1
sr  ="cushing"
s   =0.7
alb  =teleost[c("linf","k","t0","l50","a","b"),"Thunnus alalunga"]
...
```{r}
catch.n(stkr)=propagate(catch.n(stkr),dims(f)$iter)
stkr=fwd( stkr,fbar=f,sr=eq)
ref =stock.n(eq)[,1]
```

```{r fig11d, fig.height=6,fig.width=10}
scale=rnoise(1,iter(stock.wt(stkr),1),sd=0.3,b=0.9,what="cohort")
...
```{r}
scale=rnoise(1,iter(stock.wt(stkm),1),sd=0.3,b=0.9,what="cohort")
stkf=stkr
mat(stkf)=matdd(ages(iter(stock.wt(stkf),1)),alb,scale,k,TRUE) 
```
...
```{r}
stkr=fwd( stkr,fbar=f,sr=eq,
         residuals=rlnorm(1,iter(f,1)*0,.3))
```
...
```{r fig11e, fig.height=6,fig.width=10}
ggplot(dat)+
...
```{r fig12,fig.width=6,fig.height=6,fig.cap="Year effects"}
dat=rbind(cbind("Selection"="Juvenile",fishJuv),
          cbind("Selection"="Mature",  fishMat))
dat=transform(dat,F=factor(paste("F times",c(0,1,3))[iter]))
...
```{r fig13, fig.width=10,fig.height=4,fig.cap="Cohort Effects",eval=FALSE}
...
p$data=ddply(p$data,.(qname), transform, val=`50%`/mean(`50%`))

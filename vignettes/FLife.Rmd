...
Many studies have shown relationships between growth, maturity and natural mortality. Life history traits have been used for providing advice for data poor stocks, developing priors for use in data rich stock assesments and to parameterise ecological models.

`FLife` package brings together a variety of methods for modelling life history traits and ecological processes and can be used to create `FLR` objects such as `FLBRP` and `FLStock` to model species, populations or stocks. 
...
In data poor situtuations only the maximum size ($l_{max}$) may be known. Life history relationships  allow parameters for growth, maturity, natural mortality to derived from empirical relationship [@gislason2008coexistence]. The `lhPar` method uses life history relationships to fill in missing values in data poor datasets. 

$k$ of the [@vonbert1957quantitative] growth equation  
...
There are defaults for other values, which can not be derived from life history theory; these include $a$ and $b$ of the length weight relationship $w=al^b$, `ato95` the age at which 95\% of fish are mature, the offset to age at which 50\% are mature, selectivity-at-age parameters, `sl`  and `sr` the standard deviations of left and righthand limbs of the double normal and parameters for the stock recruitment relationship, `s` steepness of stock recruitment relationship, `v` virgin biomass.

Then functions for growth, maturity and natural mortality, e.g. `vonB`, `sigmoid`, and `lorezen` methods can be used. These take as their first argument an object for age, length or weight and an `FLPar` as the second argument with the life history parameters. 
...
```{r age-m}
m =FLQuants(Lorenzen=lorenzen(wt[-1],par[c("m1","m2")]),
            Gislason=gislason(ln[-1],par[-(8:9)]))
```
...
The `FLPar` objected created can be used to parameterise an `FLBRP` equilibrium object 

```{r}
eql=lhEql(par)
```

and then create an `FLStock` object to simulate time series
...
```{r, fig.margin=TRUE, fig.height=4, fig.cap="Time series",echo=FALSE}
stk=as(eql,"FLStock")
plot(stk)
```
...
```{r teleost, fig.margin=TRUE, fig.height=4, fig.cap="Parameters in teleost dataset.",echo=FALSE}
...
```{r srr, fig.margin=TRUE, fig.height=4, fig.cap="Stock recruitment relationships",echo=FALSE}
...
```{r prod, fig.margin=TRUE, fig.height=4, fig.cap="Production functions",echo=FALSE}
...
```{r fig5,eval=!TRUE,fig.height=3,fig.width=6,echo=FALSE}
...
```{r fig7, fig.margin=TRUE, fig.height=4, fig.cap="Overfishing",echo=FALSE}
...
```{r fig8, fig.margin=TRUE, fig.height=4, fig.cap="Rebuilding",echo=FALSE}
k   =1
sr  ="cushing"
s   =0.7
...
```{r fig11, fig.margin=TRUE, fig.height=4, fig.cap="a",echo=FALSE}
k   =1
sr  ="cushing"
s   =0.7
...
```{r fig12, fig.margin=TRUE, fig.height=4, fig.cap="b",echo=FALSE}
dat=rbind(cbind("Selection"="Juvenile",fishJuv),
          cbind("Selection"="Mature",  fishMat))
dat=transform(dat,F=factor(paste("F times",c(0,1,3))[iter]))
...
```{r fig13, fig.margin=TRUE, fig.height=4, fig.cap="c",echo=FALSE}
library(FLAssess)
...
```{r fig14, fig.margin=TRUE, fig.height=4, fig.cap="d",echo=FALSE}
library(FLife)
library(FLash)
library(FLBRP)
...
```{r fig15, fig.margin=TRUE, fig.height=4, fig.cap="e",echo=FALSE}

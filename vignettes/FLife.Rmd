...
Therefore for a value of $L_{\infty}$ or even $L_{max}$ the maximum size observered as $L_{\infty} ~ 0.95\L_{max}$, all the growth parameters can be recovered.
...
```{r fig7,fig.height=6,eval=FALSE}
library(FLash)
library(FLife)
...
for (i in seq(dims(fbar(eq))$year)[2:50]){
  ...
  stkf=FLash:::fwd(stkf,f=f[,i],sr=eq)}
...
```{r fig8,fig.height=6,echo=FALSE,eval=FALSE}
k   =1
sr  ="cushing"
s   =0.7
